/*  Auhtor: Maciej Rak
 *	Description: Day 2s challene was to count passwords,
 *	in the passwords list, that meet requirements specified
 *  in following patterns:
 *
 *	Silver star:  // function check()
 *  <min_occurances>-<max_occurances> <character>: <password>
 *
 *  Gold star: // function chechV2()
 *  <index0>-<index1> <character>: <passwrd>
 *  where <character> had to appear in exatctly
 *  one of the specified indexes in the <passwd>
 */
package main
...
func main() {
	...
	// Initialze number of correct passwords to 0
	var correct = 0
	...
	for _, line := range lines {
		...
		// Parse line into 4 values
		min, max, letter, passwd := parse(line)

		// Check if passwd meets specified rules
		// increment number of corrent passwords on
		// the list if true
		if checkV2(passwd, letter, min, max) {
			correct++
		}
	}
...
// Check checks if password fulfils rules specified
// in silver-star part of challenge
func check(passwd, letter string, min, max int) bool {

	// Initialze counter to 0
	counter := 0

	// Count occurances of letter in passwd
	...
	// Check if amount of numbers fit
	// in provided range
...
// Check checks if password fulfils rules specified
// in gold-star part of challenge
func checkV2(passwd, letter string, index0, index1 int) bool {

	// Checks both indexes contain specified character,
	// then it XORs executed checks, in order to make sure
	// only one of them cointains given letter
	return (string(passwd[index0-1]) == string(letter)) != (string(passwd[index1-1]) == letter)
}

// Regular expressions allowing us to simply
// and efficiently find following parts of line:
// rmin -> minimal number of occurances / index0
// rmax -> maximal number of occurances / index1
// rletter -> lettter for which we conclude check
// rpasswd -> actual password on which we conclude check
var rmin, _ = regexp.Compile("^[0-9]*-")
var rmax, _ = regexp.Compile("-[0-9]* ")
var rletter, _ = regexp.Compile(" [a-z]:")
var rpasswd, _ = regexp.Compile(": .*")

// Function allowing us to parse line
// into 4 values, needed to execute check function
func parse(line string) (int, int, string, string) {

	// Find min with regexp
	min := rmin.FindString(line)
	min = min[0 : len(min)-1]

	// Find max with regexp
	max := rmax.FindString(line)
	max = max[1 : len(max)-1]

	// Find letter with regexp
	letter := rletter.FindString(line)
	letter = letter[1 : len(letter)-1]

	// Find passwd with regexp
	passwd := rpasswd.FindString(line)
	passwd = passwd[2:]

	// prase string numbers into integers
	retMin, _ := strconv.Atoi(min)
	retMax, _ := strconv.Atoi(max)
	return retMin, retMax, letter, passwd
}

// Input data
var input = `1-4 n: nnnnn
5-7 z: qhcgzzz
7-11 m: mmmmmmsmmmmm
