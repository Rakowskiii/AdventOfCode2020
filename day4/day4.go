/*  Auhtor: Maciej Rak
 *	Description: Day 4s challene was to validate passports
 *	in the passport list, initially (silver star),
 *	the job was to validate if all needed fields are present
 *  (needed meaning all but CID)
 *
 *	Gold-star part, needed us to also verify present fields,
 *  if values in given fields match the expected pattern
 */
...
// Compile regular expressions, allowing us
// to quickly and efficiently find all needed
// fields of passport
var byrReg, _ = regexp.Compile("byr:[0-9]{4}")
var iyrReg, _ = regexp.Compile("iyr:[0-9]{4}")
var eyrReg, _ = regexp.Compile("eyr:[0-9]{4}")
var hgtReg, _ = regexp.Compile("hgt:[0-9]*((cm)|(in))")
var hclReg, _ = regexp.Compile("hcl:#[0-9a-f]{6}")
var eclReg, _ = regexp.Compile("ecl:((amb)|(blu)|(brn)|(gry)|(grn)|(hzl)|(oth))")
var pidReg, _ = regexp.Compile("pid:[0-9]{9}($|[^0-9])")

// CID is not a required field, therefore we do not need
// to check it whatsoever
// var cidReg, _ = regexp.Compile("cid:[0-9]{3}")

// Some helper regular expressions, used to
// find out what unit of measure is used (inches / centimeters)
var cmReg, _ = regexp.Compile("[0-9]+cm")
var inReg, _ = regexp.Compile("[0-9]+in")

// Helper regular expression used to extract number
// From height string
var numReg, _ = regexp.Compile("[0-9]+")

func main() {

	// Initialization of valid passport counter
	var counter = 0

	// Iteration over array of passports
	for _, i := range strings.Split(input, "\n\n") {

		// Usage of regular expressions in order to
		// find values of required fields

		// Owners birth year
		byr := byrReg.FindString(i)

		// Passports issue year
		iyr := iyrReg.FindString(i)

		// Passports expiration year
		eyr := eyrReg.FindString(i)

		// Owners height
		hgt := hgtReg.FindString(i)

		// Owners hair color
		hcl := hclReg.FindString(i)

		// Owners eye color
		ecl := eclReg.FindString(i)

		// Passports id
		pid := pidReg.FindString(i)

		// Owners country ID ( not required )
		// cid := cidReg.FindString(i)

		// Check if all required fields are valid
		// Increase number of correct ones on the list
		// if true
		if validate(byr, iyr, eyr, hgt, hcl, ecl, pid) {
			counter++
		}
	}

	// Print final number of correct passports
	fmt.Println("Number of correct passports: ", counter)
}

// Function allowing veryfication of 7 required fields' validity
func validate(byr, iyr, eyr, hgt, hcl, ecl, pid string) bool {

	// Check if all fields exist
	if hcl == "" || ecl == "" || pid == "" || byr == "" || iyr == "" || eyr == "" || hgt == "" {
		return false
	}

	// Parse owners birth year into number,
	// check if fits into provided range
	birthYear, _ := strconv.Atoi(numReg.FindString(byr))
	if birthYear < 1920 || birthYear > 2002 {
		return false
	}

	// Parse passports issue year into number,
	// check if fits into provided range
	issueYear, _ := strconv.Atoi(numReg.FindString(iyr))
	if issueYear < 2010 || issueYear > 2020 {
		return false
	}

	// Parse passports expiration year into number,
	// check if fits into provided range
	expirationYear, _ := strconv.Atoi(numReg.FindString(eyr))
	if expirationYear < 2020 || expirationYear > 2030 {
		return false
	}

	// Check either owners height is provided in
	// inches or centimeters
	// Then parse it into number, and check
	// if fits into range provided for given unit
	if inReg.FindString(hgt) != "" {
		ln, _ := strconv.Atoi(numReg.FindString(hgt))
		if ln < 59 || ln > 76 {
			return false
			//fmt.Println("invalid heigth (59-76)", hgt)
		}
	}
	if cmReg.FindString(hgt) != "" {
		ln, _ := strconv.Atoi(numReg.FindString(hgt))
		if ln < 150 || ln > 193 {
			return false
			//fmt.Println("invalid heigth (150-193)", hgt)
		}
	}

	// If none of checks returned false,
	// return true, as passport is valid
	return true
}
